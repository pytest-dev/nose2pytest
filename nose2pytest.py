...
def has_weak_op(node: PyNode) -> bool:
    for child in node.children:
        if child.type in NEWLINE_OK_TOKENS:
            return False
        ...
        # identity tests
        if isinstance(child, PyNode) and has_weak_op(child):
            return True

    return False


def group_if_non_leaf(arg_node: PyNode or PyLeaf, check: bool = True) -> PyNode or PyLeaf:
    """
    ...
    """
    if check and isinstance(arg_node, PyNode) and has_weak_op(arg_node):
...
def group_if_non_leaf2(node: PyNode or PyLeaf, check: bool = True) -> PyNode or PyLeaf:
    """
    ...
    if check and isinstance(node, PyNode):
        ...
        if first_child.type in NEWLINE_OK_TOKENS or first_child.type == token.MINUS:
            return node

        # first child does not start with ([{, but, if node represents attr access (dot something) no need for parens:
        sibling = first_child.next_sibling
        if sibling is not None and sibling.type == 332:
            return node

        # if isinstance(sibling, PyNode) and sibling.children[0].type in NEWLINE_OK_TOKENS:
        #     return node

        print('adding parens: "{}" ({}), "{}" ({})'.format(first_child, first_child.type, sibling, sibling.type))
